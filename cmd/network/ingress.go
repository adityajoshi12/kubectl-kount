...
type ingressCountCmd struct {
	out    io.Writer
	errOut io.Writer
	opts   common.Options
}

func (p *ingressCountCmd) Validate() error {
...
func (p ingressCountCmd) Run() error {

	list, err := client.GetClient().NetworkingV1().Ingresses(p.opts.Namespace).List(context.Background(), common.ListOptions)
	if err != nil {
...
func NewIngressCountCmd(out io.Writer, errOut io.Writer) *cobra.Command {
	...
	cmd :=
		&cobra.Command{
			...
			Short:   "Count ingress in a namespace.",
			Long:    `Count ingress in a namespace, optionally filtered by a label.`,
