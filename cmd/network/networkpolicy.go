...
type networkpolicyCountCmd struct {
	out    io.Writer
	errOut io.Writer
	opts   common.Options
}

func (p *networkpolicyCountCmd) Validate() error {
...
func (p networkpolicyCountCmd) Run() error {

	list, err := client.GetClient().NetworkingV1().NetworkPolicies(p.opts.Namespace).List(context.Background(), common.ListOptions)
	if err != nil {
...
func NewNetworkpolicyCountCmd(out io.Writer, errOut io.Writer) *cobra.Command {

	c := networkpolicyCountCmd{
		out:    out,
		errOut: errOut,
	}
	cmd :=
		&cobra.Command{
			...
			Short:   "Count networkpolicies in a namespace.",
			Long:    `Count networkpolicies in a namespace, optionally filtered by a label.`,
