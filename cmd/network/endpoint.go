...
type endpointCountCmd struct {
	out    io.Writer
	errOut io.Writer
	opts   common.Options
}

func (p *endpointCountCmd) Validate() error {
...
func (p endpointCountCmd) Run() error {

	list, err := client.GetClient().CoreV1().Endpoints(p.opts.Namespace).List(context.Background(), common.ListOptions)
	if err != nil {
...
func NewEndpointCountCmd(out io.Writer, errOut io.Writer) *cobra.Command {
	...
	cmd :=
		&cobra.Command{
			...
			Short:   "Count endpoints in a namespace.",
			Long:    `Count endpoints in a namespace, optionally filtered by a label.`,
