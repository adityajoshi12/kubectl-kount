...
type limitrangeCountCmd struct {
	out    io.Writer
	errOut io.Writer
	opts   common.Options
}

func (c *limitrangeCountCmd) Validate() error {
...
func (c *limitrangeCountCmd) Run() error {
	list, err := client.GetClient().CoreV1().LimitRanges(c.opts.Namespace).List(context.Background(), common.ListOptions)
	if err != nil {
...
func NewLimitrangeCmdCountCMD(out io.Writer, errout io.Writer) *cobra.Command {
	c := hpaCountCmd{
		out:    out,
		errOut: errout,
	}
	var cmd = &cobra.Command{
		Use:     "limitranges",
		Aliases: []string{"limits", "limitrange"},
		Example: "kubectl kount limitranges -n kube-system",
