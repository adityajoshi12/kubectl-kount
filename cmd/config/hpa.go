...
type hpaCountCmd struct {
	out    io.Writer
	errOut io.Writer
	opts   common.Options
}

func (c *hpaCountCmd) Validate() error {
...
func (c *hpaCountCmd) Run() error {
	list, err := client.GetClient().AutoscalingV1().HorizontalPodAutoscalers(c.opts.Namespace).List(context.Background(), common.ListOptions)
	if err != nil {
...
func NewHpaCountCmdCountCMD(out io.Writer, errout io.Writer) *cobra.Command {
	c := hpaCountCmd{
		out:    out,
		errOut: errout,
	}
	var cmd = &cobra.Command{
		...
		Example: "kubectl kount horizontalpodautoscalers -n kube-system",
