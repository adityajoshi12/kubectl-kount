...
type statefulsetCountCmd struct {
	out    io.Writer
	errOut io.Writer
	opts   common.Options
}

func (p *statefulsetCountCmd) Run() error {
	list, err := client.GetClient().AppsV1().StatefulSets(p.opts.Namespace).List(context.Background(), common.ListOptions)
	if err != nil {
...
func (p *statefulsetCountCmd) Validate() error {
...
func NewStatefulSetCountCmd(out io.Writer, errOut io.Writer) *cobra.Command {

	c := statefulsetCountCmd{
		out:    out,
		errOut: errOut,
	}
	cmd :=
		&cobra.Command{
			Use:     "statefulset",
			Aliases: []string{"ss", "statefulsets"},
