...
type cronjobCountCmd struct {
	out    io.Writer
	errOut io.Writer
	opts   common.Options
}

func (p *cronjobCountCmd) Run() error {
	list, err := client.GetClient().BatchV1().CronJobs(p.opts.Namespace).List(context.Background(), common.ListOptions)
	if err != nil {
...
func (p *cronjobCountCmd) Validate() error {
...
func NewCronJobCountCmd(out io.Writer, errOut io.Writer) *cobra.Command {
	...
	cmd :=
		&cobra.Command{
			...
			Short:   "Count cronjobs in a namespace.",
			Long:    `Count cronjobs in a namespace, optionally filtered by a label.`,
