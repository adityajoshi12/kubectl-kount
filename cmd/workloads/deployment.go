...
type deploymentCountCmd struct {
	out    io.Writer
	errOut io.Writer
	opts   common.Options
}

func (p *deploymentCountCmd) Run() error {

	list, err := client.GetClient().AppsV1().Deployments(p.opts.Namespace).List(context.Background(), common.ListOptions)
	if err != nil {
...
func (p *deploymentCountCmd) Validate() error {
...
func NewDeploymentCountCmd(out io.Writer, errOut io.Writer) *cobra.Command {

	c := deploymentCountCmd{
		out:    out,
		errOut: errOut,
	}
	cmd :=
