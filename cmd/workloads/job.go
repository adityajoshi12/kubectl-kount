...
type jobCountCmd struct {
	out    io.Writer
	errOut io.Writer
	opts   common.Options
}

func (p *jobCountCmd) Run() error {
	list, err := client.GetClient().BatchV1().Jobs(p.opts.Namespace).List(context.Background(), common.ListOptions)
	if err != nil {
...
func (p *jobCountCmd) Validate() error {
...
func NewJobCountCmd(out io.Writer, errOut io.Writer) *cobra.Command {
	...
	cmd :=
		&cobra.Command{
			Use:     "job",
			Aliases: []string{"jobs"},
			Short:   "Count jobs in a namespace.",
			Long:    `Count jobs in a namespace, optionally filtered by a label.`,
