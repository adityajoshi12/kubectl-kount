...
type replicasetCountCmd struct {
	out    io.Writer
	errOut io.Writer
	opts   common.Options
}

func (p *replicasetCountCmd) Run() error {
	list, err := client.GetClient().AppsV1().ReplicaSets(p.opts.Namespace).List(context.Background(), common.ListOptions)
	if err != nil {
...
func (p *replicasetCountCmd) Validate() error {
...
func NewReplicasetCountCmd(out io.Writer, errOut io.Writer) *cobra.Command {

	c := replicasetCountCmd{
		out:    out,
		errOut: errOut,
	}
	cmd :=
